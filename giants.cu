...
)
{
	...
	if (newmin(2*numshorts,MAX_SHORTS) > current_max_size)	// Set as comment ; J.P. 25/01/2011
 ...
 * size of the stack giants. */
	if (stack_glen == 0) {
//		current_max_size = MAX_SHORTS;		// J.P. 25/01/2011	
		stack_glen = current_max_size;
	}
	...
	else
	{
		...
		if (!error)
		{
			...
			if (thegiant->sign<0 && basetengrand->n[i]!=0)
				columnwrite(filepointer,&column,(char*)"-",0, newlines);
			columnwrite(filepointer,&column,(char*)"%d",basetengrand->n[i],newlines);
			for( ; i>0; )
			{
				--i;
				columnwrite(filepointer,&column,(char*)"%04d",basetengrand->n[i],newlines);
			}
		}
	}
   pushg(2);
...
{
	if (1/*bitlen(y)<= GCDLIMIT*/)		// ggcd does not work... J.P. 24/01/2011
		bgcdg(x,y);
	else
		ggcd(x,y);
}
...
{
	int c = gcompg (d, n);	// Test n positive and < d or d == n (J.P. 15/01/11)
	if (c == 0)
		n->sign = 0;
	if (((c == 1) && (n->sign > 0)) || (c == 0))
		return;
...
{
	register long long int 		j, k, m, car, last;
...
*/
	...
	for(j=0;j < n;j+=2)
	{
if(gfloor(z[j]+0.5) < 0.0){	//msft
 printf("\n\n\nKnown Issue.\n");
exit(4);
}
if(gfloor(z[j+1]+0.5) < 0.0){	//msft
 printf("\n\n\nKnown Issue.\n");
exit(4);
}
		m = (long long int)(gfloor(z[j]+0.5)) +(long long int)(gfloor(z[j+1]+0.5))*0x100+car;
		x->n[j>>1] = (unsigned short)(m & 0xffff);
		car = (m>>16);
	}
	j=j>>1;
	if (car)
...
/*
	//printf("0 %d",lpt(size+size, &j));
	L = lpt(size+size, &j)<<1; 
	//if(L > 262144) L = L << 2;//msft
...
*/
	{
		...
		unsigned long long * x_4;
		unsigned long long * x_5;
		size_t x_1_size;
		size_t x_5_size;
		size_t m_x_size;
		int m_x_size_byte;
		mpz_t m_x;
		x_1 = (unsigned long long*)malloc(((size+3)/4)*4*4);	
		x_3 = (unsigned long long*)malloc(((size+3)/4)*4*4);	
		x_1_size = ((size+3)/4);
		x_2 = (unsigned long long*)malloc(sizeof(unsigned long long)*(size+3));	
		for(i_1=0;i_1<size;i_1++)
			x_2[i_1]=x->n[i_1];
		x_2[size]=0;
		x_2[size+1]=0;
		x_2[size+2]=0;
		for(i_1=0;i_1<x_1_size;i_1++)
			x_1[i_1]=x_2[i_1 * 4] + x_2[i_1 * 4+1] * 0x10000 + x_2[i_1 * 4+2] * 0x100000000 + x_2[i_1 * 4+3] * 0x1000000000000 ;
		for(i_1=0;i_1<x_1_size;i_1++)
			x_3[i_1]=x_1[x_1_size-i_1-1];
		mpz_init(m_x);
		mpz_import(m_x,x_1_size,1,sizeof(unsigned long long),0,0,x_3);
		mpz_mul(m_x,m_x,m_x);
		m_x_size = mpz_size(m_x);
		m_x_size_byte = m_x_size*sizeof(unsigned long long);
		x_4 = (unsigned long long*)malloc(m_x_size_byte);	
		x_5 = (unsigned long long*)malloc(m_x_size*sizeof(unsigned long long)*4);	
		x_5_size = m_x_size*4;	
		mpz_export(x_4,NULL,0,m_x_size_byte,0,0,m_x);
		for(i_1=0;i_1<m_x_size;i_1++)
		{
			x_5[i_1*4]=x_4[i_1] & 0xffff;
			x_5[i_1*4+1]=(x_4[i_1] >> 16) & 0xffff;
			x_5[i_1*4+2]=(x_4[i_1] >> 32) & 0xffff;
			x_5[i_1*4+3]=(x_4[i_1] >> 48) & 0xffff;
		}
		while(!(x_5[x_5_size-1])) --x_5_size;
		for(i_1=0;i_1<x_5_size;i_1++)
			x->n[i_1]=x_5[i_1];
		x->sign = x_5_size;
		x->sign = abs(x->sign);
		free(x_1);
...
/*
	//printf("1 %d ",lpt(sizex+sizey, &lambda));
	L = lpt(sizex+sizey, &lambda)<<1; 
	//if(L > 262144) L = L << 2;//msft
	if(!z) z = (double *)malloc(MAX_SHORTS * sizeof(double));
...
*/
	{
		...
		mpz_t m_x;
		mpz_t m_y;
		x_1 = (unsigned long long*)malloc(((sizex+3)/4)*4*4);	
		x_3 = (unsigned long long*)malloc(((sizex+3)/4)*4*4);	
		x_1_size = ((sizex+3)/4);
		x_2 = (unsigned long long*)malloc(sizeof(unsigned long long)*(sizex+3));	
		for(i_1=0;i_1<sizex;i_1++)
			x_2[i_1]=x->n[i_1];
		x_2[sizex]=0;
		x_2[sizex+1]=0;
		x_2[sizex+2]=0;
		...
		free(x_2);
		free(x_3);
		x_1 = (unsigned long long*)malloc(((sizey+3)/4)*4*4);	
		x_3 = (unsigned long long*)malloc(((sizey+3)/4)*4*4);	
		x_1_size = ((sizey+3)/4);
		x_2 = (unsigned long long*)malloc(sizeof(unsigned long long)*(sizey+3));	
		for(i_1=0;i_1<sizey;i_1++)
			x_2[i_1]=y->n[i_1];
		x_2[sizey]=0;
		x_2[sizey+1]=0;
		x_2[sizey+2]=0;
		...
		for(i_1=0;i_1<x_1_size;i_1++)
			x_3[i_1]=x_1[x_1_size-i_1-1];
		mpz_init(m_y);
		mpz_import(m_y,x_1_size,1,sizeof(unsigned long long),0,0,x_3);
		mpz_mul(m_x,m_x,m_y);
		m_x_size = mpz_size(m_x);
		m_x_size_byte = m_x_size*sizeof(unsigned long long);
		x_4 = (unsigned long long*)malloc(m_x_size_byte);	
