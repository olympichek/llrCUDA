...
//extern int FFTLEN;
//static 	dd_real gw_fftlen_inverse = dd_real (1.0) / dd_real ((double) FFTLEN);
...
/*extern "C"
double fftcosp (unsigned long j, unsigned long N) {
        dd_real arg;
	x86_FIX
	arg = dd_real::_pi * (double) j / (double) N;
	END_x86_FIX
	return ((double)cos (arg));
}

extern "C"
double fftsinp (unsigned long j, unsigned long N) {
        dd_real arg;
	x86_FIX
	arg = dd_real::_pi * (double) j / (double) N;
...
extern "C"
double fftcosp (unsigned long j, int N) {
dd_real gw_fftlen_inverse = dd_real (1.0) / dd_real ((double) N);
return (cos (dd_real ((double)j) * dd_real::_pi * gw_fftlen_inverse));
}

extern "C"
double fftsinp (unsigned long j, int N) {
dd_real gw_fftlen_inverse = dd_real (1.0) / dd_real ((double) N);
return (sin (dd_real ((double)j) * dd_real::_pi * gw_fftlen_inverse));
}
	...
	unsigned long j)
{
	...
	bpower = gwceil (temp) - temp;
	if (! dd_data->gw__c_is_one) bpower += dd_data->gw__logb_abs_c_div_fftlen * (double) j;
	result = exp (dd_data->gw__logb * -bpower) * dd_real (0.5)  * dd_data->gw__over_fftlen;
	END_x86_FIX
	return (double (result));
}
